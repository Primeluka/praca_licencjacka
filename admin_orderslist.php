...
$sth = $pdo->query('SELECT DISTINCT session_id FROM orders WHERE status=2 ORDER BY date ASC');
$count=$sth->rowCount();
...
if ($count > 0) {
    ...
    foreach ($sth->fetchAll() as $value) {
        ...
        echo '<br><form action="admin_orderslist_done.php" method="post">
            <input type="hidden" name="session_id" value='. $value['session_id'] .'>
            <input type="submit" name="confirm" value="Gotowe" />
        </form><hr>';

        $count-=1;
    }
} else {
    echo 'Brak aktualnych zamówień.';
}

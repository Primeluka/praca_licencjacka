...
$sth = $pdo->query('SELECT DISTINCT session_id FROM orders WHERE status=4 ORDER BY date DESC');
$count=$sth->rowCount();

if ($count > 0) {
    echo '<h2 class="text-danger">Historia zamówień</h2>';

    foreach ($sth->fetchAll() as $value) {
        $tbl=$pdo->query('SELECT products.name, products.details, products.price, orders.date, categories.name AS category_name FROM orders INNER JOIN products INNER JOIN categories ON orders.product_id=products.id AND categories.id=products.category_id WHERE session_id="'.$value['session_id'].'"');
        echo '<h4 class="text-danger">Zamówienie nr '. $count.'</h4>';
        echo '<h6 class="text-danger">Status zamówienia: Dostarczono</h6>';
        ...
        echo '<h5 class="text-center">Zapłacono: '. $totalPrice . ' zł</h5><br></div><hr>';
